    ...
    <div style={{ minHeight: '100vh', background: '#040C18', fontFamily: 'Inter, system-ui, sans-serif', color: '#F0F6FF' }}>
      ...
      <header style={{ background: 'rgba(4,12,24,0.95)', borderBottom: '1px solid rgba(26,86,219,0.15)', backdropFilter: 'blur(16px)', position: 'sticky', top: 0, zIndex: 50, padding: '0 24px', height: '60px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <nav style={{ display: 'flex', gap: '4px' }}>
            {[
              ...
              <a key={item.href} href={item.href} style={{ padding: '5px 12px', borderRadius: '6px', fontSize: '0.78rem', color: item.href === '/crm' ? '#60A5FA' : '#8BA8CC', background: item.href === '/crm' ? 'rgba(26,86,219,0.12)' : 'transparent', textDecoration: 'none', fontWeight: item.href === '/crm' ? 600 : 400 }}>
                {item.label}
              </a>
            ))}
          </nav>
        ...
        <button
          ...
          style={{ padding: '8px 16px', background: 'linear-gradient(135deg,#1A56DB,#1E40AF)', border: 'none', borderRadius: '8px', color: 'white', fontSize: '0.8rem', fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit', boxShadow: '0 4px 16px rgba(26,86,219,0.4)' }}
        >
          + Novo cliente
        </button>
      </header>
